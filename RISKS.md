...
| P1 | Zero-stake or bad-parameter deployment | Bad constructor inputs can brick an instance; no eligible or registered stake can leave reward rounds unclaimable until cleanup rules apply. | Deployment-time validation and operator runbooks. |
...
| P1 | Expiry window misconfiguration | Too-short claim durations can prevent active token holders from registering, and no-registration rounds are recyclable by anyone. | Deployment runbooks, UI warnings, and tests for deadline behavior. |
...
- **Expiring token rounds are active-voter rounds.** A nonzero immutable claim duration records a deadline for each
  funded token round. Before the deadline, token holders register snapshot voting power; after the deadline, only
  registered voting power shares the pot. A token round with no registered voters can be recycled into the current
  reward round.
...
- **Undelegated `IVotes` balances dilute only non-expiring token rounds.** Token distributors with
  ...
  use registered snapshot votes, so undelegated or non-registering balances do not share the active-voter pot.
...
- **Expired recycling is permissionless but deadline-gated.** Any caller can recycle eligible expired inventory, but
  non-expired rounds, non-expiring rounds, and active-voter token rounds with registered voters recycle zero.
...
- **Token distributor hooks must be IVotes-compatible.** Non-expiring token funding records the current reward round's
  `getPastTotalSupply` snapshot. Active-voter token registration records each claimant's `getPastVotes` snapshot.
  Arbitrary non-IVotes hook addresses are not valid token-distributor hooks.
...
- round snapshots stay stable within a round once initialized, including zero-balance ones; active-voter token rounds can
  grow `totalStake` only through pre-deadline registrations at the fixed snapshot block
- expired recycling settles eligible expired rounds and records the recycled amount into the current round without changing
  tracked balance; active-voter token rounds with registered voters do not recycle
...
- late claim transactions recycle expired token rounds only when no active voters registered; registered active voters
  can still vest their share after the deadline
...
Deployers can set a claim duration to attach a deadline to all funding paths. The window starts when the funded reward round first becomes claimable, not when the transfer lands. For token distributors, a nonzero duration is an active-voter registration window: holders register snapshot votes before the deadline and registered voters claim after it. After the deadline, `recycleExpiredRewards` can be called by anyone, but active-voter token rounds with registered voters recycle zero.

This is intentionally different from late non-expiring claims. Non-expiring rounds remain reserved for historical stakers or NFT owners indefinitely. Active-voter token rounds trade total-supply dilution for an explicit registration window; rounds with no registrations can be cleaned up permissionlessly after the deployer's configured window.
