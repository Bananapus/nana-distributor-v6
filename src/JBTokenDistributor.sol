...
contract JBTokenDistributor is JBDistributor, IJBTokenDistributor {
    ...
    /// @notice Directly fund this token distributor with rewards that expire if unclaimed.
    /// @dev The claim window starts when the funded round first becomes claimable.
    /// @param hook The IVotes token whose stakers receive the rewards.
    /// @param token The reward token being funded.
    /// @param amount The nominal amount to fund. Ignored for native ETH; `msg.value` is used instead.
    /// @param claimDuration The number of seconds stakers have to claim once the round becomes claimable.
    ...
    /// @notice Burn unclaimed rewards from expired token-distributor reward rounds.
    /// @param hook The IVotes token whose expired rewards should be burned.
